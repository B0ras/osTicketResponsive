...
if ($page && $_REQUEST['a'] != 'add') {
    ...
    $translations = CustomDataTranslation::allTranslations(
        $page->getTranslateTag('name:body'),
        'article'
    );
...
<form action="pages.php?<?php echo Http::build_query($qs); ?>" method="post" class="save">
    ...
    <div style="margin-top: 10px">
        <ul class="tabs clean">
            <li class="active"><a href="#page-content">
        ...
        <div class="tab_content active" id="page-content">
            ...
            if ($page && count($langs) > 1) { ?>
                <ul class="tabs alt clean" id="translations">
                    ...
                    <?php foreach ($langs as $tag => $nfo) { ?>
                        <li class="<?php if ($tag == $cfg->getPrimaryLanguage())
                        echo "active";
            ...
            <div id="translations_container">
                <div id="translation-<?php echo $cfg->getPrimaryLanguage(); ?>" class="tab_content"
                    ...
                    <textarea name="body" cols="21" rows="12" class="richtext draft" data-width="<?php echo $width; ?>"
                        <?php
                        if (!$info['type'] || $info['type'] == 'thank-you')
                            echo 'data-root-context="thank-you"';
                ...
                foreach ($langs as $tag => $nfo) {
                    if ($tag == $cfg->getPrimaryLanguage())
                        ...
                        <div id="translation-<?php echo $tag; ?>" class="tab_content hidden"
                            ...
                            <textarea name="trans[<?php echo $tag; ?>][body]" cols="21" rows="12" <?php if ($info['type'] == 'thank-you')
                               echo 'data-root-context="thank-you"'; ?> style="width:100%"
                                class="richtext draft" data-width="<?php echo $width; ?>" <?php
    ...
    <p style="text-align:center">
        ...
        <input type="button" name="cancel" value="<?php echo __('Cancel'); ?>"
            onclick='window.location.href="pages.php"'>
    </p>
</form>