...
<form action="profile.php" method="post" class="save" autocomplete="off">
  ...
  <div class="tab_content table-responsive" id="account">
    <table class="table two-column" width="940" border="0" cellspacing="0" cellpadding="2">
              ...
              <div class="avatar pull-left" style="margin: 10px 15px; width: 100px; height: 100px;">
                ...
                if ($avatar->isChangeable()) { ?>
                  <div style="text-align: center">
                    <a class="button no-pjax" href="#ajax.php/staff/<?php echo $staff->getId(); ?>/avatar/change" onclick="javascript:
      ...
      <tbody>
        ...
        if (($bks = Staff2FABackend::allRegistered())) {
          ...
          ?>
          <tr>
            <td <?php if ($required2fa)
              echo 'class="required"'; ?>>
              <?php echo __('Default 2FA'); ?>:
            </td>
            <td>
              <select name="default_2fa" id="default2fa-selection" style="width:300px">
                <?php
                if (!$required2fa) { ?>
                ...
                foreach ($bks as $bk) {
                  ...
                  ?>
                  <option id="<?php echo $bk->getId(); ?>" value="<?php echo $bk->getId(); ?>" <?php
        ...
        <tr>
          <td colspan="2">
            <label class="checkbox">
              <input type="checkbox" name="onvacation" <?php echo ($staff->onvacation) ? 'checked="checked"' : ''; ?> />
  ...
  <div class="hidden tab_content" id="preferences">
    <table class="table two-column" width="100%">
      <tbody>
            ...
            <select name="default_from_name">
              ...
              foreach ($options as $k => $v) {
                echo sprintf(
                  ...
                  $k, ($staff->default_from_name && $staff->default_from_name == $k) ? 'selected="selected"' : '',
                  $v
                );
              }
              ?>
            ...
            <select name="default_ticket_queue_id">
              ...
              foreach ($queues as $q) { ?>
                <option value="<?php echo $q->id; ?>" <?php
                   if ($q->getId() == $staff->default_ticket_queue_id)
                     echo 'selected="selected"'; ?>>
                  <?php echo $q->getFullName(); ?>
                </option>
              <?php
              } ?>
            </select>
          ...
          <td>
            <select name="default_signature_type">
              ...
              foreach ($options as $k => $v) {
                echo sprintf(
                  '<option value="%s" %s>%s</option>',
                  $k, ($staff->default_signature_type == $k) ? 'selected="selected"' : '',
                  $v
                );
              }
              ?>
            ...
            <div class="error">
              <?php echo $errors['default_signature_type']; ?>
            </div>
          </td>
        </tr>
        <tr>
          ...
          <td>
            <select name="default_paper_size">
              ...
              foreach (Export::$paper_sizes as $v) {
                echo sprintf(
                  '<option value="%s" %s>%s</option>',
                  $v, ($staff->default_paper_size == $v) ? 'selected="selected"' : '',
                  __($v)
                );
              }
              ?>
            ...
            <div class="error">
              <?php echo $errors['default_paper_size']; ?>
            </div>
          </td>
        </tr>
        <tr>
          <td>
            <?php echo __('Reply Redirect'); ?>:
          ...
          <td>
            <select name="reply_redirect">
              ...
              foreach ($options as $key => $opt) {
                echo sprintf(
                  '<option value="%s" %s>%s</option>',
                  $key, ($staff->reply_redirect == $key) ? 'selected="selected"' : '',
                  $opt
                );
              }
              ?>
        ...
        <tr>
          <td>
            <?php echo __('Image Attachment View'); ?>:
          ...
          <td>
            <select name="img_att_view">
              ...
              foreach ($options as $key => $opt) {
                echo sprintf(
                  '<option value="%s" %s>%s</option>',
                  $key, ($staff->img_att_view == $key) ? 'selected="selected"' : '',
                  $opt
                );
              }
              ?>
          ...
          <td>
            <select name="editor_spacing">
              ...
              foreach ($options as $key => $opt) {
                echo sprintf(
                  '<option value="%s" %s>%s</option>',
                  $key, ($spacing == $key) ? 'selected="selected"' : '',
                  $opt
                );
              }
              ?>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            <select name="datetime_format">
              ...
              foreach (array('relative' => __('Relative Time'), '' => '— ' . __('System Default') . ' —', ) as $v => $name) { ?>
        ...
        <?php if (extension_loaded('intl')) { ?>
          <tr>
            <td>
              <?php echo __('Preferred Locale'); ?>:
            </td>
            <td>
              <select name="locale">
                <option value="">
                  <?php echo __('Use Language Preference'); ?>
                </option>
  ...
  <div id="signature" class="hidden">
    <table class="table two-column" width="100%">
      <tbody>
        ...
        <tr>
          <td colspan="2">
            <textarea class="richtext no-bar" name="signature" cols="21" rows="5"
              style="width: 60%;"><?php echo $staff->signature; ?></textarea>
          </td>
        </tr>
      </tbody>
    </table>
