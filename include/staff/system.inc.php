...
?>
<h2>
    <?php echo __('About this osTicket Installation'); ?>
</h2>
    ...
    <table class="list" width="100%" ;>
        ...
        <tbody>
            <tr>
                ...
                <td><span class="ltr">
                        <?php
                        echo sprintf("%s (%s)", THIS_VERSION, trim($commit)); ?>
                    </span>
                    <?php
                    ...
                    if ($lv && $tv[0] == 'v' ? version_compare(THIS_VERSION, $lv, '>=') : $lv == $gv) { ?>
                        — <span style="color:green"><i class="icon-check"></i>
            ...
            <tr>
                <td>
                    <?php echo __('Web Server Software'); ?>
                </td>
                <td><span class="ltr">
        ...
        <tbody>
            <tr>
                ...
                echo ini_get('cgi.fix_pathinfo') == 1 ? 'check' : 'warning-sign'; ?>"></i>
                    <span class="faded">
                        <?php echo __('"1" is recommended if AJAX is not working'); ?>
                    </span>
                </td>
            </tr>
            <tr>
        ...
        <thead>
            <tr>
                <th colspan="2">
                    <?php echo __('Database Information and Usage'); ?>
                </th>
            </tr>
        </thead>
        <tbody>
            <tr>
                ...
                <td>
                    <?php echo sprintf('<span class="ltr">%s (%s)</span>', DBNAME, DBHOST); ?>
                </td>
            </tr>
            </tr>
            <tr>
                <td>
                    <?php echo __('Schema Signature'); ?>
                </td>
                <td><?php echo $cfg->getSchemaSignature(); ?> </td>
            </tr>
            <tr>
                <td>
                    <?php echo __('Space Used'); ?>
                </td>
                <td><?php
            ...
            <tr>
                <td>
                    <?php echo __('Space for Attachments'); ?>
                </td>
                <td><?php
                $sql = 'SELECT
...
<div style="margin: 0 20px">
    ...
    foreach (Internationalization::availableLanguages() as $info) {
        ...
        ?>
        <h3><strong>
                <?php echo Internationalization::getLanguageDescription($info['code']); ?>
            </strong>
            <?php if ($manifest) { ?>
        ...
        <div>
            <?php echo sprintf(
                '<code>%s</code> — %s', $info['code'],
                str_replace(ROOT_DIR, '', $info['path'])
            ); ?>
            <?php if ($manifest) { ?>
                <br />
                <?php echo __('Version'); ?>: <?php echo $manifest['Version'];
                   ?>, <?php echo sprintf(
                       __('for version %s'),
