                ...
                <tbody>
                    ...
                    foreach ($ost->plugins->allInstalled() as $p) {
                        if (!$p instanceof Plugin)
                            continue; ?>
                        <tr>
                            <td align="center"><input type="checkbox" class="ckb" name="ids[]"
                                    value="<?php echo $p->getId(); ?>" <?php echo $sel ? 'checked="checked"' : ''; ?>></td>
                            ...
                            <td>
                                <?php echo $p->getVersion(); ?>
                            </td>
                            <td><?php echo ($p->isActive())
                                ? 'Enabled' : '<strong>Disabled</strong>'; ?></td>
                            <td>
                                <?php echo Format::datetime($p->getInstallDate()); ?>
                            </td>
                        </tr>
                    <?php } ?>
                </tbody>
                <tfoot>
                    <tr>
                        <td colspan="5">
                            ...
                            <?php } else {
                                echo sprintf(
                                    __('No plugins installed yet &mdash; %s add one %s!'),
                                    '<a href="?a=add">',
                                    '</a>'
                                );
                            } ?>
        ...
        <p class="confirm-action" style="display:none;" id="delete-confirm">
            <font color="red"><strong>
                    <?php echo sprintf(
                        ...
                        _N('selected plugin', 'selected plugins', 2)
                    ); ?>
                </strong></font>
            <br><br><?php echo __(
