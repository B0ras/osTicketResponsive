...
<form action="filters.php?<?php echo Http::build_query($qs); ?>" method="post" class="save">
                        ...
                        <select name="target">
                            ...
                            foreach (Filter::getTargets() as $k => $v) {
                                echo sprintf(
                                    ...
                                    $k,
                                    (($k == $info['target']) ? 'selected="selected"' : ''),
                                    $v
                                );
                            }
    ...
    <ul class="clean tabs" style="margin-top:20px;" id="filter-tabs">
        <li class="active"><a href="#filter_rules"><i class="icon-filter"></i>
                <?php echo __('Filter Rules'); ?>
            </a></li>
        <li><a href="#filter_actions"><i class="icon-bolt"></i> <?php echo __('Filter Actions'); ?></a></li>
        <li><a href="#internal_notes"><i class="icon-file-text-alt"></i>
                        ...
                        <td colspan="2">
                            <select style="max-width: 200px;" name="rules[<?php echo $i; ?>][w]">
                                ...
                                foreach ($matches as $group => $ms) { ?>
                                    <optgroup label="<?php echo __($group); ?>">
                                        <?php
                                        foreach ($ms as $k => $v) {
                            ...
                            <input type="text" size="60" name="rules[<?php echo $i; ?>][v]"
                                value="<?php echo $rule["v"]; ?>">
                            <div class="pull-right" style="padding-right:20px;"><a href="#" class="clearrule"
                                    onclick="javascript: $(this).closest('tr').remove();">(<?php echo __('clear'); ?>)</a>
                            </div>
                            <div class="error">
                                <?php echo $errors["rule_$i"]; ?>
                            </div>
                        </td>
                    </tr>
                ...
                if ($filter) {
                    foreach ($filter->getActions() as $A) {
                        ...
                        <tr style="background-color:white">
                            <td><i class="icon-sort icon-large icon-muted"></i>
            ...
            <select name="new-action" id="new-action-select"
                ...
                foreach (FilterAction::allRegistered() as $group => $actions) {
                    if ($group && $current_group != $group) {
                        ...
                        $current_group = $group;
                        ?>
                        <optgroup label="<?php echo Format::htmlchars($group); ?>">
                        <?php
                    }
    ...
    <p style="text-align:center;">
        ...
        <input type="button" name="cancel" value="<?php echo __('Cancel'); ?>"
            onclick='window.location.href="filters.php"'>
    </p>
</form>
