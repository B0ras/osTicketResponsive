...
if (($search instanceof SavedQueue) && !$search->checkOwnership($thisstaff)) {
    ...
    if ($matches) {
        ?>
        <div id="ticket-flags" style="padding:5px; border-top: 1px dotted #777;">
            <strong><i class="icon-caret-down"></i>&nbsp;
                <?php
                echo __('Supplemental Filters'); ?>
            </strong>
        </div>
    <?php
    }
...
foreach ($form->getFields() as $name => $field) {
    ...
    if ($sub === 'search') {
        ...
        echo sprintf(
            '<div class="adv-search-field-body %s">',
        ...
        <?php if (!$has_errors && $sub === 'search' && isset($info[$name]) && $info[$name]['active']) { ?>
            <span style="padding-left: 5px">
                <a href="#" data-name="<?php echo Format::htmlchars($name); ?>" onclick="javascript:
                    ...
                    <span class="faded">
                        <?php echo $search->describeField($info[$name]); ?>
                    </span>
                </a>
            </span>
        <?php } ?>
...
if ($matches && is_array($matches)) { ?>
    ...
    <div class="d-flex align-items-center justify-content-between">
        ...
        <select class="form-select" id="search-add-new-field" name="new-field" style="max-width: 300px;">
            <option value="">—
                <?php echo __('Add Other Field'); ?> —
            </option>
            <?php
