...
?>
<li class="row top-queue item <?php if ($child_selected)
  echo 'child active';
elseif ($selected)
  echo 'active'; ?>">
  <a href="<?php echo $this_queue->getHref(); ?>" class="Ticket col">
    <?php echo $this_queue->getName(); ?>
    <?php if ($cfg->showTopLevelTicketCounts()) { ?>
      <span id="queue-count-bucket" class="hidden">
        (<span class="queue-count" data-queue-id="<?php echo $this_queue->id; ?>"><span class="faded-more"></span>
        </span>)
      </span>
    <?php } ?>
  </a>
  <i class="icon-sort-down pull-right col-1"></i>
  <div class="customQ-dropdown col-7">
    <ul class="scroll-height">
      ...
      if (!$children) { ?>
        <li class="top-level">
          <span class="pull-right newItemQ queue-count" data-queue-id="<?php echo $q->id; ?>"><span
              class="faded-more">-</span>
          </span>

          <a class="truncate <?php if ($selected)
            echo ' active'; ?>" href="<?php echo $q->getHref();
    ...
    <div class="add-queue">
      <a class="full-width" onclick="javascript:
        ...
        <span><i class="green icon-plus-sign"></i>
          <?php echo __('Add personal queue'); ?>
        </span>
      </a>
    </div>
  </div>
