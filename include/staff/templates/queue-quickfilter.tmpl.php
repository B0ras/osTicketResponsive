...
<span class="action-button muted" data-dropdown="#quickfilter-dropdown">
    ...
    <span><i class="icon-filter"></i>
        <?php
        echo $qf_field->get('label');
        if (isset($quick_filter) && isset($choices[$quick_filter]))
            echo sprintf(': %s', Format::htmlchars($choices[$quick_filter])); ?>
    </span>
</span>


<div id="quickfilter-dropdown" class="action-dropdown anchor-right" onclick="javascript:
...
return false;">
    <ul <?php if ($choices && count($choices) > 20)
        echo 'style="height:500px;overflow-x:hidden;overflow-y:scroll;"'; ?>>
        <?php if ($choices) {
