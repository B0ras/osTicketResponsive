...
?>
<h2 style="margin:0 0 10px 2px;">
    <?php
    echo sprintf('<a href="emails.php">%s</a>', __('Emails'));
    if ($email) {
        echo sprintf(
            '<small> &mdash; <a href="emails.php?id=%d">%s</a></small>',
            $email->getId(),
...
<form action="emails.php?<?php echo Http::build_query($qs); ?>" method="post" class="save">
    ...
    <ul class="tabs">
        <li class="active"><a id="account_tab" href="#account"><i class="icon-envelope"></i>&nbsp <?php echo __('Account'); ?></a></li>
        <?php
        if ($email) { ?>
            <li><a id="mailbox_tab" href="#mailbox"><i class="icon-inbox"></i>&nbsp;<?php echo __('Remote Mailbox');
            ?></a></li>
            <li><a id="smtp_tab" href="#smtp"><i class="icon-reply-all"></i>&nbsp;<?php echo sprintf(
                '%s (%s)',
                __('Outgoing'),
                __('SMTP')
            );
        ...
        <table class="form_table" width="940" border="0" cellspacing="0" cellpadding="2">
            ...
            <tbody>
                <tr>
                    ...
                    <td>
                        <input type="text" size="35" name="email" value="<?php echo $info['email']; ?>" autofocus>
                    ...
                    <td>
                        <span>
                            <select name="topic_id">
                                <option value="0" selected="selected">&mdash; <?php echo __('System Default'); ?>
                                    &mdash;</option>
                                <?php
                                $warn = '';
                                ...
                                foreach ($topics as $id => $topic) { ?>
                                    <option value="<?php echo $id; ?>" <?php echo ($info['topic_id'] == $id) ? 'selected' : ''; ?>>
                                        <?php echo $topic; ?>
                                    </option>
                                    <?php
                                } ?>
                ...
                <tr>
                    <td colspan=2>
                        <textarea class="richtext no-bar" name="notes" cols="21" rows="5"
                            style="width: 60%;"><?php echo $info['notes']; ?></textarea>
                    </td>
                </tr>
            </tbody>
        </table>
