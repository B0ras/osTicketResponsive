...
<form action="emailsettings.php" method="post" class="save">
        ...
        <table class="form_table settings_table" width="940" border="0" cellspacing="0" cellpadding="2">
            <thead>
                <tr>
                    <th colspan="2">
                        <em>
                            <?php echo __('Note that some of the global settings can be overridden at department/email level.'); ?>
                        </em>
                    </th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    ...
                    <td>
                        <select name="default_template_id">
                            <option value="">&mdash; <?php echo __('Select Default Email Template Set'); ?> &mdash;
                            </option>
                            <?php
                ...
                <tr>
                    <td width="180" class="required">
                        <?php echo __('Default System Email'); ?>:
                    </td>
                    <td>
                        <select name="default_email_id">
                            ...
                            if (($res = db_query($sql)) && db_num_rows($res)) {
                                while (list($id, $email, $name) = db_fetch_row($res)) {
                                    ...
                                    ?>
                                    <option value="<?php echo $id; ?>" <?php echo ($config['default_email_id'] == $id) ? 'selected="selected"' : ''; ?>>
                                        <?php echo $email; ?>
                                    </option>
                                    <?php
                                }
                ...
                <tr>
                    <td width="180" class="required">
                        <?php echo __('Default Alert Email'); ?>:
                    </td>
                    <td>
                        <select name="alert_email_id">
                            <option value="0" selected="selected"><?php echo __('Use Default System Email (above)'); ?>
                            </option>
                            <?php
                            ...
                            if (($res = db_query($sql)) && db_num_rows($res)) {
                                while (list($id, $email, $name) = db_fetch_row($res)) {
                                    $email = $name ? "$name &lt;$email&gt;" : $email;
                                    ?>
                                    <option value="<?php echo $id; ?>" <?php echo ($config['alert_email_id'] == $id) ? 'selected="selected"' : ''; ?>>
                                        <?php echo $email; ?>
                                    </option>
                                    <?php
                                }
                ...
                <tr>
                    <td width="180" class="required">
                        <?php echo __("Admin's Email Address"); ?>:
                    </td>
                    <td>
                ...
                <tr>
                    <td width="180" class="required">
                        <?php echo __("Verify Email Addresses"); ?>:
                    </td>
                    <td>
                        <input type="checkbox" name="verify_email_addrs" <?php
                        if ($config['verify_email_addrs'])
                ...
                <tr>
                    <td width="180">
                        <?php echo __('Strip Quoted Reply'); ?>:
                    </td>
                    <td>
                ...
                <tr>
                    <td width="180">
                        <?php echo __('Reply Separator Tag'); ?>:
                    </td>
                    <td><input type="text" name="reply_separator" value="<?php echo $config['reply_separator']; ?>">
                        &nbsp;<font class="error">&nbsp;<?php echo $errors['reply_separator']; ?></font>&nbsp;<i
                            class="help-tip icon-question-sign" href="#reply_separator_tag"></i>
                    </td>
                </tr>
                <tr>
                    <td width="180">
                        <?php echo __('Emailed Tickets Priority'); ?>:
                    </td>
                    <td>
                ...
                <tr>
                    <td width="180">
                        <?php echo __('Accept Email Collaborators'); ?>:
                    </td>
                ...
                <tr>
                    <th colspan=2><em><strong>
                            ...
                            </strong>: <?php echo __('Default email only applies to outgoing emails without SMTP setting.'); ?></em></th>
                </tr>
                <tr>
                    <td width="180">
                        <?php echo __('Default MTA'); ?>:
                    </td>
                    <td>
                        <select name="default_smtp_id">
                            <option value=0 selected="selected">
                                <?php echo __('None: Use PHP mail function'); ?>
                            </option>
                            <?php
                            ...
                            foreach ($accounts as $account) {
                                ...
                                <option value="<?php echo $id; ?>" <?php
                                   echo ($config['default_smtp_id'] == $id) ? 'selected="selected"' : ''; ?>>
                                    <?php
                                    echo $email; ?>
                                </option>
                                <?php
    ...
    <p class="text-center">
        ...
        <input class="btn btn-sm btn-outline-warning small-margin-top" type="reset" name="reset" value="<?php echo __('Reset Changes'); ?>">
    </p>
</form>